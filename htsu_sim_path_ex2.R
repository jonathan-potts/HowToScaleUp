###############################################################################
# Name: htsu_sim_path_ex2.R
#
# Purpose: Simulate correlated path through a resource layer
...
beta<-1.5       # beta_1 from Equation (S.4) in Supplementary Appendix A
kappa<-0.5        # kappa from Equation (S.4) in Supplementary Appendix A
lambda<-0.2     # lambda from Equation (S.4) in Supplementary Appendix A
...
for(step in 2:step_no)
{
  ...
  alpha_z<-atan2(y_vals-loc_y[step-1],x_vals-loc_x[step-1])
  unnorm_mk<-exp(-lambda*sqrt((x_vals-loc_x[step-1])^2+(y_vals-loc_y[step-1])^2)+beta*layer+kappa*cos(alpha_x-alpha_z))
  ...
  # Set the bearing
  alpha_x<-atan2(loc_y-loc_y[step-1],loc_x-loc_x[step-1])
}
